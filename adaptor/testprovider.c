...
int main() {
	...
	addr.family = AF_NNET;
	addr.prefix = "/Rpi-1/lab1038/info/about";
	...
	while (1) {
		...
		// ret = decap_pkt(recvbuf, MAX_IBUF_LEN, &ibuf, sizeof(ibuf));
		// if (ret < 0) {
		// 	printf("interest decap error.\n");
		// 	exit(1);
		// }
		...
		// int len = encap_data(ibuf.name, ibuf.nlen, content, clen, sendbuf, MAX_DBUF_LEN);
...
// int main() {
// 	int sock = socket(AF_NNET, SOCK_RAW, NIPPROTO_ILISTEN);
// 	if (sock < 0) {
// 		printf("socket create error.\n");
// 		exit(1);
// 	}
...
// 	// addr.enable_mask = BIND_PREFIX;
// 	// addr.family = AF_NNET;
// 	// addr.prefix = "/mixsim/real/demo";
// 	// addr.plen = strlen(addr.prefix);
// 	addr.family = AF_NNET;
// 	addr.enable_mask = BIND_FACEID;
// 	addr.face_id = 0x03;
...
// 	uint cnt = 0;
// 	while (1) {
// 		ret = recv(sock, recvbuf, MAX_IBUF_LEN, 0);
// 		if (ret < 0) {
// 			printf("interest recv error.\n");
// 			exit(1);
// 		}
